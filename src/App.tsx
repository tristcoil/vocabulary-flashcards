...
function App(): JSX.Element {
  ...
  const handleLoginSuccess = (credentialResponse: CredentialResponse) => {
    const details = jwt_decode<any>(credentialResponse.credential);
    setUser(details);
    Cookies.set("username", details.name, { expires: 7 });
    Cookies.set("email", details.email, { expires: 7 });
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {user ? (
        <div>
          <div>
            <h1 className="text-xl font-bold mb-4">React variables:</h1>
            <h2 className="text-lg mb-2">You are logged in as {user.name}</h2>
            <p>Email: {user.email}</p>
          </div>
          <div className="mt-8">
            <h1 className="text-xl font-bold mb-4">Cookie variables:</h1>
            <h2 className="text-lg mb-2">
              You are logged in as {Cookies.get("username")}
            </h2>
        ...
        <GoogleOAuthProvider clientId={clientId}>
          <h1 className="text-xl font-bold mb-4">
            Vite + React Flashcard learning demo
          </h1>
          <GoogleLogin
